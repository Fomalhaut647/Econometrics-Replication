...
def generate_table3(df):
    ...
    # Row 3: Change in mean FTE employment (using all available data)
    # 这里的关键修正：第三行应该使用前两行的均值差，而不是个体变化的均值
    row3 = {}
    for group_name in groups.keys():
        # 计算均值差 = Row2的均值 - Row1的均值
        if not pd.isna(row2[group_name]['mean']) and not pd.isna(row1[group_name]['mean']):
            mean_change = row2[group_name]['mean'] - row1[group_name]['mean']
            # 标准误使用两个独立均值差的公式
            se_change = calculate_difference_se(row2[group_name]['se'], row1[group_name]['se'],
                                                row2[group_name]['se'], row1[group_name]['se'])
            # 样本数使用较小的那个（因为是基于两个独立的均值）
            n_change = min(row1[group_name]['n'], row2[group_name]['n'])
        else:
            mean_change, se_change, n_change = np.nan, np.nan, 0

        row3[group_name] = {'mean': mean_change, 'se': se_change, 'n': n_change}
...
def write_output(results):
    ...
    with open(output_path, 'w') as f:
        ...
        f.write(
            f"{format_cell(results['row1_diffs']['nj_pa_diff']['mean'], results['row1_diffs']['nj_pa_diff']['se'])} | ")
        f.write(f"{format_cell(results['row1']['NJ_wage_425']['mean'], results['row1']['NJ_wage_425']['se'])} | ")
        f.write(
            f"{format_cell(results['row1']['NJ_wage_426_499']['mean'], results['row1']['NJ_wage_426_499']['se'])} | ")
        f.write(
            f"{format_cell(results['row1']['NJ_wage_500plus']['mean'], results['row1']['NJ_wage_500plus']['se'])} | ")
        f.write(
            f"{format_cell(results['row1_diffs']['low_high_diff']['mean'], results['row1_diffs']['low_high_diff']['se'])} | ")
        f.write(
            f"{format_cell(results['row1_diffs']['mid_high_diff']['mean'], results['row1_diffs']['mid_high_diff']['se'])} |\n")
        ...
        f.write(
            f"{format_cell(results['row2_diffs']['nj_pa_diff']['mean'], results['row2_diffs']['nj_pa_diff']['se'])} | ")
        f.write(f"{format_cell(results['row2']['NJ_wage_425']['mean'], results['row2']['NJ_wage_425']['se'])} | ")
        f.write(
            f"{format_cell(results['row2']['NJ_wage_426_499']['mean'], results['row2']['NJ_wage_426_499']['se'])} | ")
        f.write(
            f"{format_cell(results['row2']['NJ_wage_500plus']['mean'], results['row2']['NJ_wage_500plus']['se'])} | ")
        f.write(
            f"{format_cell(results['row2_diffs']['low_high_diff']['mean'], results['row2_diffs']['low_high_diff']['se'])} | ")
        f.write(
            f"{format_cell(results['row2_diffs']['mid_high_diff']['mean'], results['row2_diffs']['mid_high_diff']['se'])} |\n")
        ...
        f.write(
            f"{format_cell(results['row3_diffs']['nj_pa_diff']['mean'], results['row3_diffs']['nj_pa_diff']['se'])} | ")
        f.write(f"{format_cell(results['row3']['NJ_wage_425']['mean'], results['row3']['NJ_wage_425']['se'])} | ")
        f.write(
            f"{format_cell(results['row3']['NJ_wage_426_499']['mean'], results['row3']['NJ_wage_426_499']['se'])} | ")
        f.write(
            f"{format_cell(results['row3']['NJ_wage_500plus']['mean'], results['row3']['NJ_wage_500plus']['se'])} | ")
        f.write(
            f"{format_cell(results['row3_diffs']['low_high_diff']['mean'], results['row3_diffs']['low_high_diff']['se'])} | ")
        f.write(
            f"{format_cell(results['row3_diffs']['mid_high_diff']['mean'], results['row3_diffs']['mid_high_diff']['se'])} |\n")
        ...
        f.write(
            f"{format_cell(results['row4_diffs']['nj_pa_diff']['mean'], results['row4_diffs']['nj_pa_diff']['se'])} | ")
        f.write(f"{format_cell(results['row4']['NJ_wage_425']['mean'], results['row4']['NJ_wage_425']['se'])} | ")
        f.write(
            f"{format_cell(results['row4']['NJ_wage_426_499']['mean'], results['row4']['NJ_wage_426_499']['se'])} | ")
        f.write(
            f"{format_cell(results['row4']['NJ_wage_500plus']['mean'], results['row4']['NJ_wage_500plus']['se'])} | ")
        f.write(
            f"{format_cell(results['row4_diffs']['low_high_diff']['mean'], results['row4_diffs']['low_high_diff']['se'])} | ")
        f.write(
            f"{format_cell(results['row4_diffs']['mid_high_diff']['mean'], results['row4_diffs']['mid_high_diff']['se'])} |\n")
        ...
        f.write(
            f"{format_cell(results['row5_diffs']['nj_pa_diff']['mean'], results['row5_diffs']['nj_pa_diff']['se'])} | ")
        f.write(f"{format_cell(results['row5']['NJ_wage_425']['mean'], results['row5']['NJ_wage_425']['se'])} | ")
        f.write(
            f"{format_cell(results['row5']['NJ_wage_426_499']['mean'], results['row5']['NJ_wage_426_499']['se'])} | ")
        f.write(
            f"{format_cell(results['row5']['NJ_wage_500plus']['mean'], results['row5']['NJ_wage_500plus']['se'])} | ")
        f.write(
            f"{format_cell(results['row5_diffs']['low_high_diff']['mean'], results['row5_diffs']['low_high_diff']['se'])} | ")
        f.write(
            f"{format_cell(results['row5_diffs']['mid_high_diff']['mean'], results['row5_diffs']['mid_high_diff']['se'])} |\n")
